...
const GamingHero = memo(() => (
  <div className="relative w-full h-[200px] bg-gradient-to-br from-[#111] to-[#0a0a0a] rounded-3xl overflow-hidden border border-white/10 shadow-lg flex items-center justify-center">
    ...
    <h2 className="absolute bottom-4 text-white text-xl font-black tracking-widest uppercase z-20">
      Nexus Online
    </h2>
  </div>
));

const GamingMenu = memo(
  ({
    isOpen,
    onClose,
    onNavigate,
      ...
      <div className="absolute inset-0 flex items-center px-4 md:px-6 z-10">
        ...
        <div className="flex-1 min-w-0 flex flex-col justify-center">
          ...
          <span className="text-[10px] font-bold uppercase text-white/40">
            {game.category} &bull; Lvl {game.level}
          </span>
        </div>
        <div className="text-right">
          <span className="text-sm font-bold text-cyan-400">
            {Math.round(xpPercentage)}% XP
          </span>
        </div>
      </div>
...
const GamingPage: React.FC<GamingPageProps> = ({
  onMenuClick,
  onWalletClick,
  onBack,
  balance: propBalance,
  ...
  const filteredGames =
    activeTab === "all"
      ? GAMES
      : GAMES.filter(
          (g) => g.category.toLowerCase() === activeTab.toLowerCase(),
        );
        ...
        <div className="w-full max-w-4xl mx-auto px-4 md:px-8 pb-16">
          ...
          <div className="flex items-center gap-4 overflow-x-auto pb-6 mb-4 no-scrollbar">
            {CATEGORIES.map((cat) => (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-full border transition-all whitespace-nowrap ${activeTab === cat.id ? "bg-white text-black" : "bg-[#111] border-white/10 text-white/50"}`}
              >
                <cat.icon size={16} />
          ...
          <div className="space-y-4">
            {filteredGames.length > 0 ? (
              ...
              <div className="text-center py-20 opacity-20 uppercase font-black tracking-[0.5em]">
                No Games Found
              </div>
            )}
          </div>
