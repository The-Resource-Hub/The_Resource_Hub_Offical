...
// --- LIGHTWEIGHT GAMING HERO (CSS/Framer Motion) ---
const GamingHero = memo(() => {
  return (
    <div className="relative w-full h-[200px] bg-gradient-to-br from-[#111] to-[#0a0a0a] rounded-3xl overflow-hidden border border-white/10 shadow-lg flex items-center justify-center">
      {/* Background glow and subtle animation */}
      <motion.div 
      ...
      <motion.div 
        ...
        className="absolute inset-0 border-t-2 border-r-2 border-cyan-400/20 rounded-3xl"
      />
      ...
      <motion.h2 
        ...
        className="absolute bottom-4 text-white text-xl font-black tracking-widest uppercase z-20"
      >
        Nexus Online
      </motion.h2>
    </div>
...
// GamePillCard is memoized for performance
...
const GamingPage: React.FC<GamingPageProps> = ({ onMenuClick, onWalletClick, onBack, balance }) => {
  ...
  const filteredGames = activeTab === 'all' 
    ? GAMES 
    : GAMES.filter(g => g.category.toLowerCase().includes(activeTab) || g.category.toLowerCase() === activeTab);
    ...
    <div className="flex flex-col h-screen bg-[#0a0a0a] overflow-hidden relative">
          ...
          <AnimatePresence>
            {isMenuOpen && (
              <>
                ...
                <GamingMenu isOpen={isMenuOpen} onClose={() => setIsMenuOpen(false)} onNavigate={handleMenuNavigate} />
              </>
            )}
          </AnimatePresence>
        </div>
        <h1 className="text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-500 to-cyan-500 tracking-widest">
            GAMING ZONE
        </h1>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        <div className="w-full max-w-4xl mx-auto px-4 md:px-8 pb-16">
          ...
          {/* GAMING HERO (CSS/Framer Motion) */}
          <div className="w-full h-[200px] mt-6 mb-8">
            <GamingHero />
          </div>
            ...
            {CATEGORIES.map(cat => (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-full border transition-all whitespace-nowrap ${
                  activeTab === cat.id ? 'bg-white text-black' : 'bg-[#111] border-white/10 text-white/50'
                }`}
              >
                <cat.icon size={16} />
          ...
          <div className="space-y-4">
            {filteredGames.map((game) => (
              <GamePillCard key={game.id} game={game} />
            ))}
          </div>
