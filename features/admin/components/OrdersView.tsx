...
export const OrdersView: React.FC<OrdersViewProps> = ({ orders, onHandover }) => {
    const [orderSubTab, setOrderSubTab] = useState<'all' | 'confirmed' | 'deliver'>('all');
    const [deliverSearchId, setDeliverSearchId] = useState('');
                ...
                <tbody className="divide-y divide-white/5">
                    {data.map((order) => (
                        <tr key={order.id} className="hover:bg-white/[0.02] transition-colors group">
                            <td className="p-6">
                                <span className="font-mono text-xs font-bold text-white/70">{order.id}</span>
                                <div className="text-[10px] text-white/30 mt-1">{order.date}</div>
                            </td>
                            <td className="p-6">
                                <div className="font-bold text-white text-sm">{order.user}</div>
                                <div className="text-[10px] text-white/40">{order.items} items</div>
                            </td>
                            ...
                            <td className="p-6 text-right">
                                <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button className="p-2 rounded-lg bg-white/5 hover:bg-white/10 text-white/50 hover:text-white transition-colors"><AdminIcons.Eye /></button>
                                </div>
                            </td>
                        </tr>
                    ))}
    ...
    const handleSearchOrder = () => {
        const order = orders.find(o => o.id.toLowerCase() === deliverSearchId.toLowerCase().trim());
        setFoundOrder(order || null);
    };

    const confirmHandover = () => {
        if (foundOrder) {
            onHandover(foundOrder.id);
            setFoundOrder((prev: any) => ({ ...prev, status: 'Completed' }));
            setDeliverSearchId('');
        }
    };
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            ...
            <div className="flex flex-wrap gap-2">
                ...
                <button
                    ...
                    className={`px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${orderSubTab === 'confirmed' ? 'bg-cyan-500 text-black' : 'bg-white/5 text-white/40 hover:bg-white/10'}`}
                >
                    Confirmed
                </button>
            </div>
